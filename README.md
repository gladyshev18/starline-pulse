...
В отчёт входят поездки, пробег, время в пути, топливо и расход, работа двигателя без движения, прогревы, заправки, состояние АКБ и последний снимок состояния автомобиля. Те же сводки доступны командами `/day`, `/week` и `/month`.

## TODO

- Добавить импорт чеков из почты по IMAP, сопоставление писем с событиями заправки и защиту от повторного импорта.
- Добавить распознавание содержимого чеков (сумма, цена и объём топлива) после подключения IMAP; сейчас файлы хранятся без разбора.
...
Снимки состояния автомобиля, поездки, журнал обращений к API и прикреплённые к заправкам чеки хранятся бессрочно. Файлы чеков находятся в каталоге `RECEIPT_STORAGE_DIR` (по умолчанию `data/refuel-receipts`). В приложении нет фоновой очистки, TTL или ограничения истории по дате. Журнал поездок выводится постранично, поэтому все накопленные поездки остаются доступны независимо от их возраста.
