...
// Страница длинная — два десятка карточек, — поэтому разделы вынесены в
// оглавление над ней. Ссылки обычные, якорные: переход по ним отдан браузеру,
// а отступ под липкими панелями держит `scroll-margin-top`.
const sections = [
...
const currentSection = ref<string>(sections[0].id)

onMounted(() => {
  // Полоса наблюдения — треть экрана под липкими панелями: раздел считается
  // текущим, пока в неё попадает хоть какая-то его часть. Ниже в списке
  // побеждает последний вошедший, поэтому при прокрутке вниз отметка
  // переходит вовремя, а не когда предыдущий раздел уже уехал.
...
<template>
  <div>
    ...
    <div v-else class="history-body">
      <nav class="section-nav" aria-label="Разделы статистики">
        <a
          v-for="section in sections"
          :key="section.id"
          class="section-nav__link"
          :class="{ 'section-nav__link--current': section.id === currentSection }"
          :href="`#${section.id}`"
          :aria-current="section.id === currentSection ? 'true' : undefined"
        >{{ section.title }}</a>
      </nav>

      <section id="month" class="stats-section" aria-labelledby="month-title">
        <h2 id="month-title" class="stats-section__title">Итоги месяца</h2>
        <div class="grid">
      ...
      <section id="trends" class="stats-section" aria-labelledby="trends-title">
        <h2 id="trends-title" class="stats-section__title">Динамика и деньги</h2>
        <div class="grid">
          ...
          <section class="card card--wide history-chart-card">
            ...
            <template v-else>
              ...
              <p class="metric-meta">
                <template v-if="!yearComparison">Сравнить с прошлым годом пока не с чем: тех же месяцев год назад в данных ещё нет. </template>
                Прогноз на декабрь — простой перенос среднего дня на остаток года, сезонности он не знает.
              </p>
          ...
          <section class="card card--wide">
            ...
            <template v-if="overpay && overpay.fills">
              ...
              <p class="metric-meta">
                Каждая заправка сравнивается с ценой самой дешёвой сети, известной на её собственный день.
            ...
            <div class="card__top">
              <div>
                ...
                <p class="muted">
                  ...
                  воздух. Средняя считается по часам, а не по замерам, и поправлена по архиву погоды. Сутки, часть которых
                  машина провела в разъездах, восстановлены по суточному ходу соседних — подсказка говорит, какие именно
                </p>
              </div>
            </div>
      ...
      <section id="engine" class="stats-section" aria-labelledby="engine-title">
        <h2 id="engine-title" class="stats-section__title">Двигатель и холод</h2>
        <div class="grid">
          <section class="card card--wide">
            <div class="card__top">
              <div>
          ...
          <section class="card card--wide">
            <div class="card__top">
              <div>
                ...
                <p class="muted">
                  По пускам после ночной стоянки видно, на сколько холод удлиняет прогрев
                </p>
              </div>
            </div>
      ...
      <section id="driving" class="stats-section" aria-labelledby="driving-title">
        <h2 id="driving-title" class="stats-section__title">Как ездим</h2>
        <div class="grid">
          <section class="card card--wide">
            <div class="card__top">
              <div>
          ...
          <section class="card card--wide">
            ...
            <template v-else>
              ...
              <p class="metric-meta">
                ...
                Разброс баков шире их погрешности — это разная дорога, а не разное измерение.
              </p>
            </template>
          </section>
      ...
      <section id="quality" class="stats-section" aria-labelledby="quality-title">
        <h2 id="quality-title" class="stats-section__title">Качество данных</h2>
        <div class="grid">
          <section class="card card--wide">
            <div class="card__top">
              <div>
