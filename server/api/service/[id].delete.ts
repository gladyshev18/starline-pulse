...
export default defineEventHandler(async (event) => {
  ...
  const where = and(eq(serviceEvents.id, id), eq(serviceEvents.vehicleId, vehicle.id))
  const [existing] = await database.select().from(serviceEvents).where(where).limit(1)
  if (!existing) throw createError({ statusCode: 404, statusMessage: 'Запись не найдена' })

  await database.delete(serviceEvents).where(where)
  return { id }
})
