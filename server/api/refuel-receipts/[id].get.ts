...
import { resolveReceiptPath } from '../../utils/refuel-receipts'
...
export default defineEventHandler(async (event) => {
  ...
  const fallbackExtension = receipt.originalName.includes('.') ? receipt.originalName.slice(receipt.originalName.lastIndexOf('.')) : ''
  setResponseHeader(event, 'Content-Type', receipt.mimeType)
  setResponseHeader(event, 'Content-Length', fileSize)
  setResponseHeader(event, 'Content-Disposition', `attachment; filename="receipt${fallbackExtension}"; filename*=UTF-8''${encodeURIComponent(receipt.originalName)}`)
